...
void V2Fader::begin() {
  ...
  _adc = new V2Analog::ADC(V2Analog::ADC::getADC(_pin.wiper));
  _adc->begin();
  _adc->selectChannel(V2Analog::ADC::getChannel(_pin.wiper));
  _adc->setContinuous();
}
...
void V2Fader::tick() {
  const float fraction = _adc->read() / 1023.f;
  measure(1.f - fraction);
